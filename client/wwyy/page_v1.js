...
BookApp.util.player = (function () {
  ...
  MusicPlay.prototype = {
    ...
    setIcon: function () {
      if (this.is_paused) {
        $('#play_btn').attr('src', '/img/edu-wy-play.png');
      } else {
        $('#play_btn').attr('src', '/img/edu-wy-pause.png');
      }
    },
...
BookApp.util.dom = (function () {
  var options = {
    ...
    setBreadCrumb = function (args) {
      ...
      // console.log(args);
      var itC = args["classification"].split("|"),
        itC2 = args["classification2"].split("|"),
		    itC3 = args["classification3"] ? args["classification3"].split("|") : null;

      // console.log(args, itC2);
      
      if (itC[1]) {
        var itemCodeText = itC[0].indexOf("搜索结果") < 0 ? "" 
          : "&itemCodeText=" + encodeURIComponent(encodeURIComponent(itC[0]));

        nav_str += '<a class="split" href="'+options.host_prefix+'/?itemCode=' + itC[1] 
          + itemCodeText
          + token
          + '">'+ itC[0] +'</a>';
      }
      if (itC2[0]) {
        nav_str += '<a href="'+options.host_prefix+'/?itemCode='+ itC[1]
          + '&itemSubCode=' + itC2[1] + token
          +'"> &gt; '+ itC2[0] +'</a>';
      }
      if (itC3) {
        itC3[0] =  itC3[0].replace("-a-n-d-","&");
        var data_text = encodeURIComponent(encodeURIComponent(args["classification"] 
          + "|" + itC3[0] 
          + "|" + args["classification2"]));
          
        nav_str += '<a href="'+options.host_prefix+'/?groupId='+ itC3[1] + '|' + data_text 
          + '&groupType=' + itC3[2] 
          + '">'+ itC3[0] +'</a>'
      }	  
      if (args["bookname"]) {
        nav_str += '<a href="#"> &gt; '+ args["bookname"] +'</a>'
      }
    ...
    backHome = function (args) {
      ...
      nav_str = '',
      token = args.token ? '&token=' + args.token : '',
      url_str = options.host_prefix + '/?1=1';
      url_str += token;
      ...
      if (itC[1]) {
        var itemCodeText = itC[0].indexOf("搜索结果") < 0 ? "" 
          : "&itemCodeText=" + encodeURIComponent(encodeURIComponent(itC[0]));
        url_str +=  itemCodeText + '&itemCode=' + itC[1];
      }
      if (itC2[0]) {
          url_str += '&itemSubCode=' + itC2[1];
      }
      ...
      if (itC3) {
        ...
        //   + '&groupType=' + itC3[2] 
        //   + '">'+ itC3[0] +'</a>'
        url_str +=  '&groupId=' + itC3[1] + '|' + data_text + '&groupType=' + itC3[2] ;
      }	  
      // if (args["bookname"]) {
      //   nav_str += '<a href="#"> &gt; '+ args["bookname"] +'</a>'
      // }
    ...
    centerCurrentThumbNail = function (pIndex, previewList, host) {
      ...
      //存储尾部索引
      setPrevSrc(newPrevs);
      setActivePreviewBgColor('#fff', 'rgb(94, 100, 160)', index);
    },
    ...
    playBtnReset = function (page_wrap, body_width) {
      ...
      options.wrap_btn.style.top = (page_wrap.top + navHeight + padding_height) + 'px';
      options.wrap_btn.style.right = margin + 'px';
      options.wrap_btn.style.width = options.wrap_next.clientWidth + 'px';
      
      options.wrap_back_btn.style.top = (page_wrap.top + navHeight + padding_height) + 'px';
...
BookApp.util.page = (function () {
  ...
  var dom = BookApp.util.dom,
    ...
    fixImgRatio = function (imgw, imgh) {
      ...
      // console.log('[HD]', containerW, containerH, contentW, contentH);
      ...
      var bannerHeight = $('#nav_link').height();
    ...
    getInitData = function (args) {
      $.ajax({
        async: true,
        // userId=8394&
        url: options.data_url + args + '&userId=' + options.location_args.wawayayauserId,
        type: "GET",
    ...
    _requestDictionaryData = function (region, callback) {
      var url = 'https://testprovider-joyreader.wawayaya.com/lenovo/dictionary?userId='
        + options.location_args.wawayayauserId;
    ...
    init = function () {
      ...
      console.log(options.location_args, 
        options.location_args.deviceid);
      // deviceid
      ...
      if (options.location_args.classification ||
        options.location_args.classification2) {
        // 面包屑导航字符
